package com.example.commontask.view
...
import com.example.commontask.controller.LocationServices
import com.example.commontask.model.*
import com.example.commontask.view.adapter.ForecastListAdapter
import com.example.commontask.view.mappers.ForecastDataMapper
import com.example.commontask.view.mappers.ForecastItemMapper
...
class UserInterface(private val context: Context) {
    ...
    // Snackbar
    private var retrySnackBar = Snackbar.make(swipeRefreshLayout, "Ανεπαρκής υπηρεσία δεδομένων καιρού.", Snackbar.LENGTH_INDEFINITE)
    ...
    fun updateUI(forecastDataModel: ForecastDataModel, isDataComingFromInternet: Boolean, isHourly: Boolean? = null) {
        ...
        if (isDataComingFromInternet) {
            toast("Τα δεδομένα καιρού ανανεώθηκαν")
        }
